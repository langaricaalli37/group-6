// Pac-Man vs Bug Game (Processing Java Mode) — with Death
...
void setup() {
  ...
  bug = new Bug(int(random(cols)) * TILE_SIZE, int(random(rows)) * TILE_SIZE, color(0, 200, 0), "chase");
}
...
void draw() {
  ...
  if (!gameOver) {
    ...
    // Switch bug mode every few seconds
    modeTimer++;
  ...
  // Display mode text or Game Over
  fill(0, 255, 255);
  textSize(20);
  textAlign(LEFT, TOP);
  if (!gameOver) {
    text("Bug Mode: " + bug.mode.toUpperCase(), 10, 10);
  } else {
    fill(255, 0, 0);
    textSize(40);
...
boolean collides(PacMan p, Bug b) {
  ...
  return distance < TILE_SIZE * 0.5; // overlap threshold
}
